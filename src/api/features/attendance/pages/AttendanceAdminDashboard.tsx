...
import { Building2, Users, LayoutGrid, Activity, Filter, SwitchCamera, Calendar, Heart, Zap, Crown, TrendingUp, Users2, BarChart3 } from "lucide-react";
...
export default function AttendanceAdminDashboard() {
  ...
  return (
    <div className="space-y-8 p-4 bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800 min-h-screen">
      {/* Header + mode switch */}
      <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        ...
        <div className="flex items-center gap-4 bg-white/50 dark:bg-slate-800/50 rounded-full px-4 py-2 backdrop-blur-sm">
          ...
          >
            ...
            <span className="hidden sm:inline">{mode === "my" ? "Org View" : "My View"}</span>
          </motion.button>
      ...
      <motion.div 
        ...
        className="rounded-2xl border border-slate-200/50 dark:border-slate-700/50 p-6 bg-white/70 dark:bg-slate-800/40 backdrop-blur-sm shadow-lg space-y-4"
      >
        <div className="flex items-center gap-2">
          <Filter className="w-5 h-5 text-amber-500" />
          <div className="font-semibold text-slate-700 dark:text-slate-200">Filters & Selection</div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          />
          {mode === "my" && (
            <ChurchInput label="Church" value={myChurchId || "—"} />
          )}
        </div>
        ...
        {mode === "org" && isSiteAdmin && (
          <div className="space-y-3">
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400 italic">
              Select any level (national / district / church). Leaving deeper levels empty aggregates up.
            </div>
          </div>
        )}
      </motion.div>
      ...
      <motion.div 
        ...
        className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-8 gap-4"
      >
        {cardsConfig.map((card, i) => (
          <motion.div
            ...
            custom={i}
            className="group relative rounded-xl p-4 bg-white/80 dark:bg-slate-800/60 border border-slate-200/50 dark:border-slate-700/50 shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden"
          >
            ...
            <div className="relative z-10 flex flex-col items-center space-y-2 text-center">
              <motion.div 
                ...
                className={`w-10 h-10 rounded-lg flex items-center justify-center bg-gradient-to-r from-white to-slate-100 dark:from-slate-700 dark:to-slate-600 shadow-sm ${card.color}`}
              >
                <card.icon className="w-5 h-5 text-white" />
              </motion.div>
              <div className="font-bold text-lg text-slate-800 dark:text-slate-100 tabular-nums">
