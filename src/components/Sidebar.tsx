  ...
  return (
    <>
      <motion.aside
        ...
        animate={{ width: sidebarWidth }} // Sidebar width animation
        transition={{ type: "spring", stiffness: 300, damping: 30 }}
        className={`fixed lg:relative z-50 h-screen bg-gradient-to-b from-white/95 via-white/80 to-slate-100/70 dark:from-slate-900/95 dark:via-slate-800/80 dark:to-slate-700/70 backdrop-blur-3xl border-r border-gradient-to-b from-slate-200/50 to-slate-300/30 dark:from-slate-700/50 dark:to-slate-600/30 flex flex-col shadow-2xl shadow-slate-200/50 dark:shadow-slate-900/50 overflow-hidden transition-transform duration-300 lg:translate-x-0 ${sidebarPosition}`}
        style={{ width: isMobile ? "280px" : undefined }} // Fixed width on mobile/tablet
      >
        ...
        >
         <motion.div
          ...
          >
            <Link
              to="/"
              className="w-10 h-10 rounded-xl bg-gradient-to-br cursor-pointer inline-flex items-center justify-center shadow-red-500/20 relative z-10"
            >
          ...
          {navItems.map((item) => (
            <div key={item.name} className="space-y-0.5">
              {item.type === "link" ? (
                <NavLink
                  to={item.to!}
                  onClick={() => isMobile && setMobileOpen(false)} // Close on mobile nav
                  className={({ isActive }) =>
