...
export default function HeroChristian() {
  ...
  const container = {
    ...
    visible: {
      opacity: 1,
      transition: { delayChildren: reduce ? 0 : 0.4, staggerChildren: reduce ? 0 : 0.07 },
    },
  };
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(180deg, rgba(212,175,55,0.06) 0%, rgba(139,0,0,0.12) 50%, rgba(2,19,71,0.55) 100%)",
        }}
      />
      <div
        className="pointer-events-none absolute inset-0"
        style={{ background: "radial-gradient(circle at 50% 50%, rgba(212,175,55,0.18) 0%, transparent 58%)" }}
      />
      <div className="pointer-events-none absolute inset-0 shadow-[inset_0_-160px_240px_-100px_rgba(2,19,71,.70)]" />

      {/* bottom wave (smaller on mobile) */}
      <div className="absolute inset-x-0 bottom-0 h-8 sm:h-14 overflow-hidden" aria-hidden>
      ...
      >
        ...
        <div className="mx-auto max-w-[60rem] rounded-2xl bg-slate-900/40 backdrop-blur-[3px] ring-1 ring-white/10 p-4 sm:p-6 md:bg-transparent md:backdrop-blur-0 md:ring-0">
          {/* icon */}
          ...
          <h1
            ...
            style={{ fontSize: "clamp(1.8rem, 6vw, 3.6rem)", lineHeight: 1.12, letterSpacing: ".2px" }}
          >
          ...
          {/* FEATURES: hidden on mobile, only show on sm+ */}
          <motion.ul
            variants={container}
            initial="hidden"
            animate="visible"
            className="hidden sm:grid mt-8 mx-auto w-full max-w-[56rem] grid-cols-2 md:grid-cols-3 gap-3"
          >
            {FEATURES.map((label) => (
              <motion.li
                key={label}
