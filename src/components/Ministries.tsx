...
type Ministry = {
  ...
  icon: LucideIcon;
  color?: string;
  href?: string;      // we’ll fill this with /ministries/:slug
  slug?: string;      // add slug so we can link + lookup details
};

const makeSlug = (s: string) => s.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");

const RAW_MINISTRIES: Omit<Ministry, "href" | "slug">[] = [
...
const MINISTRIES: Ministry[] = RAW_MINISTRIES.map((m) => {
  const slug = makeSlug(m.title);
  return { ...m, slug, href: `/ministries/${slug}` };
});

export default function Ministries({ items = MINISTRIES }: { items?: Ministry[] }) {
  const reduce = useReducedMotion();

  return (
    <section id="ministries"
      aria-labelledby="ministries-title"
      className="relative overflow-hidden bg-gradient-to-br from-white via-amber-50/40 to-red-50 dark:from-slate-950 dark:via-red-950/10 dark:to-amber-950/20 py-16 sm:py-20"
    >
      {!reduce && (
      ...
      <div className="text-center mb-10 sm:mb-14">
        <motion.h2
          id="ministries-title"
          className="text-3xl md:text-4xl font-serif text-slate-900 dark:text-white"
        ...
        <motion.p
          className="mt-3 text-base sm:text-lg text-slate-600 dark:text-slate-300 max-w-2xl mx-auto"
      ...
      {!reduce && <Sparkles className="absolute bottom-12 left-1/2 -translate-x-1/2 text-amber-500/40 w-10 h-10" />}
    </section>
  );
}

export { MINISTRIES }; // export to reuse in detail page map if you want
